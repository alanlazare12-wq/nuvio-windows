import { ExternalLink, KeyRound, Mail, MessageCircle, Phone, Smartphone } from "lucide-react";
import type { TelegramAuthSnapshot } from "../types";
import "./AuthDeliveryOptions.css";

export type AuthDelivery = "telegram" | "sms" | "email" | "call" | "fragment";
const methods = [
  ...
  { id: "fragment", title: "Fragment", description: "Código mediante Fragment/TON", icon: ExternalLink },
] as const;
...
export function deliveryFamily(kind?: string | null): AuthDelivery | null {
  ...
  return kind === "telegram" || kind === "email" || kind === "fragment" ? kind : null;
}
...
export function AuthDeliveryOptions({ selected, snapshot, remaining, busy, onSelect }: {
  ...
  if (beforePhone) {
    explanation = selected === "email"
      ...
      : selected === "fragment"
        ? "Fragment solo puede usarse cuando Telegram lo ofrece. Si aparece como canal activo, Nuvio abrirá la URL oficial entregada por Telegram."
    ...
    explanation = selected === "email" && snapshot.stage === "email"
      ...
      : selected === "fragment"
        ? "Telegram habilitó Fragment para este acceso. Abre la URL oficial que Nuvio mostrará debajo para obtener el código."
    ...
    explanation = selected === "email"
      ...
      : `Telegram no ofrece ${selected === "sms" ? "SMS" : selected === "call" ? "llamada" : selected === "fragment" ? "Fragment" : "mensaje en la app"} en este intento. Puedes elegir otro canal que figure como disponible.`;
  }
  return <div className="auth-delivery">
    ...
    <div className="auth-passkey-unavailable" data-testid="passkey-unavailable">
      <KeyRound size={16} aria-hidden="true" />
      <span>
        <strong>Passkey / Windows Hello</strong>
        <small>No disponible en clientes no oficiales: Telegram restringe el RP ID a telegram.org.</small>
      </span>
    </div>
  </div>;
}
